...
| `Alt+s` | Send it (Claude git commit + push) |
...
| `<leader>f` | Grep search (alternate) |
...
| `<leader>d` | Go to definition (alternate) |
| `gD` | Go to declaration |
| `Alt+r` | Find references |
| `<leader>r` | Find references (alternate) |
| `gI` | Go to implementation |
| `Alt+x` | Go to implementation (alternate) |
...
| `Alt+Tab` | Toggle Copilot Chat window |
...
| Keybinding | Description |
| --- | --- |
| `<leader>ut` | Run Go test package |
| `<leader>uf` | Run Go test function |
...
**Alt key combos** - I use Alt for frequent LSP actions (definition, references, grep). I got used to using them in VSCode. Plus they're slightly faster than leader combos. Each Alt binding has a `<leader>` alternate for when I'm on a non-macOS keyboard.
