...
1. Add the topic as a GitHub repo secret:
   - **Terminal**: `gh secret set COFFEE_SUB`
   - **Browser/mobile**: Repo → Settings → Secrets and variables → Actions → New repository secret → Name: `COFFEE_SUB` (GitHub mobile app does not support secrets — use a phone browser in desktop mode)
...
- `_config_secrets.yml` is generated during CI from the `COFFEE_SUB` secret
...
3. Add the app to `_pages/apps.md`
